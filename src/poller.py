...
class Poller:
    ...
    def _handle_join(self, cid: str, title: str):
        ...
        if status == "pending":
            # Notify group that we're waiting for admin approval
            self.groups.send_pending(cid)
            # Notify admin immediately
            info    = self.groups._data.get(cid, {})
            title_g = info.get("title", cid)
            members = info.get("members", 0)
            self.groups.notify_admin_pending(cid, title_g, members)
            log.info(f"[{cid}] Pending admin approval — notified admin")
    ...
    def _on_callback(self, cb: dict):
        ...
        if data == "dashboard":
            ...
            if self.groups.approve(gid):
                self._answer_cb(cb["id"], "✅ Approved! Posts at 06:00 UTC")
                self._edit_msg(
                    cid, mid,
                    f"✅ <b>Approved</b>\n"
                    f"📌 {title}\n"
                    f"🆔 <code>{gid}</code>\n"
                    f"⏰ Posts at 06:00 • 12:00 • 18:00 • 00:00 UTC\n"
