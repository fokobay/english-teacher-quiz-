...
def run_quiz(chat_id: str, quiz_num: int) -> bool:
    ...
    for i, q in enumerate(questions, 1):
        ...
        if i == 10:
            # BUG FIX: original code used `continue` here which skipped the
            # inter-question sleep(3) — correct.  But it also caused the loop
            # to jump straight to i=11 WITHOUT any pause after the halfway
            # message itself, which is fine.  The real bug was that `continue`
            # appeared AFTER `_run_one` which already sleeps POLL_SECONDS, so
            # the extra sleep(3) at the bottom was the only thing being skipped.
            # Replaced with an explicit short pause and then `continue` so the
            # halfway board is shown but the redundant 3-second gap is avoided.
            time.sleep(1)
            api.send(chat_id, api.txt_halfway(scores.session_board(chat_id)))
            # Skip the trailing sleep(3) — go straight to next question
            continue

        # BUG FIX: inter-question gap only for questions that are NOT the
        # halfway marker (handled above with continue).
        time.sleep(3)
...
def on_poll_answer(poll_id: str, user_id: int, name: str, option_ids: list):
    # BUG FIX: previously the lock was held while calling scores.record(),
    # which does disk I/O.  This blocked the Poller thread for every answer.
    # Now we extract the needed data under the lock, then release before I/O.
    with _poll_lock:
