...
def send_poll(chat_id, question, options, correct_index, explanation="") -> dict | None:
    """
    BUG FIX: Previously returned the raw Telegram `result` object which is a
    Message dict containing a nested `poll` key.  quiz_pipeline._run_one()
    accessed result.get("message_id") and result.get("poll", {}).get("id")
    directly on the returned value — that is correct ONLY if we return the
    Message dict (result), not the entire API response.  Confirmed: _post()
    already calls r.json().get("result") so the return value IS the Message.
    No change needed in the return path, but the explanation below clarifies
    the contract so future callers do not wrap it again.

    Returns: Telegram Message object (dict) with keys:
        - message_id  (int)
        - poll        (dict with key "id")
    or None on failure.
    """
    payload = dict(
...
def txt_final(scores: list, num: int, total: int, total_q: int = 20) -> str:
    ...
    for i, row in enumerate(scores[:10], 1):
        name, uid, correct, _ = row
        m   = MEDALS[i-1] if i <= 3 else f"{i}."
        # BUG FIX: ZeroDivisionError if total_q is 0 (edge case with generator
        # returning < 20 questions).  Guard with max(total_q, 1).
